...
const anime_backend = require("./anime_backend")
...
app.get("/channel_fh264_getvideo", (req, res) => {
    ...
    if(yt2009_exports.getStatus(req.query.v)) {
        ...
        yt2009_exports.waitForStatusChange(req.query.v, () => {
            try {
                res.redirect("http://" + req.headers.host + "/assets/" + req.query.v + ".mp4")
            }catch(error) {}
        })
        return;
    }
...
app.get("/stream_flv", (req, res) => {
    ...
    req.on('close', () => {
        console.log(`[stream_flv] PSP disconnected for ${id}. Terminating transcode.`);
...
app.get("/anime/api/videos", (req, res) => {
    ...
    let startIndex = parseInt(req.query["start-index"]);
    let bypage = parseInt(req.query["max-results"]);
    if (!isNaN(bypage)) {
        maxResults = bypage;
    }
    if (!isNaN(startIndex) && !isNaN(maxResults)) {
        page = Math.floor((startIndex - 1) / maxResults) + 1;
    }

    anime_backend.anime_search(q, page, maxResults, (xml) => {
...
app.get("/anime/play", (req, res) => {
    let episodeId = req.query.episode_id;
    if (!episodeId) {
        res.sendStatus(400);
        return;
    }
    anime_backend.anime_play(episodeId, req, res);
});
...
app.get("/anime/thumb", (req, res) => {
    ...
    anime_backend.anime_thumb(img, res);
});
    ...
    switch(uploadType) {
        ...
        case "video_still": {
            ...
            let v = req.body.toString().split(`name="picked_video_still_id"`)[1]
                       .split("\n--")[0].trim().split("\n").join("");
            const fetch = require("/home/azureuser/psp-streaming-yt2009/back/proxy-fetch.js")
...
app.get("/exp_hd", (req, res) => {
    ...
    && fs.existsSync(`../assets/${id}-720p.mp4`)) {
        res.redirect(`http://${req.headers.host}/assets/${id}-720p.mp4?ac=${Math.random()}`)
        return;
    }

    if(fs.existsSync(`../assets/${id}-${quality}.mp4`)
    && fs.statSync(`../assets/${id}-${quality}.mp4`).size > 5) {
        res.redirect(`http://${req.headers.host}/assets/${id}-${quality}.mp4?ac=${Math.random()}`)
    } else {
        yt2009_utils.saveMp4_android(id, (success) => {
            if(success) {
                res.redirect("http://" + req.headers.host + "/assets/" + success + "?ac=" + Math.random())
            } else {
                if(lower) {
    ...
    {"send": function(data) {
        try {
            if(data && data.avatar) {
                ...
                res.redirect("http://" + req.headers.host + "/assets/site-assets/default.png")
            }
        }
        catch(error) {
            console.log(error)
