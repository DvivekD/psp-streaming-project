...
module.exports = {
    ...
    */
    ...
    "saveMp4": function(id, callback, extended) {
        ...
        let dlcmd = `yt-dlp --proxy socks5://127.0.0.1:40000 --cookies cookies.txt --extractor-args "youtube:player_client=mweb" -f "bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best" "https://www.youtube.com/watch?v=${id}" -o "${__dirname}/../assets/${id}.mp4"`;
        console.log("yt-dlp downloading " + id);
        child_process.exec(dlcmd, (err, stdout, stderr) => {
            console.log("yt-dlp done for " + id);
            callback(`../assets/${id}`)
        });
    },
    ...
    "saveMp4_android": function(id, callback, existingPlayer, quality, replayerCount) {
        ...
        function parseResponse(r) {
            ...
            // forced DASH for PSP hardware transcoding
            ...
            function onFormatsDone() {
                ...
                let cmd = [
                    ...
                    `-vcodec libx264 -profile:v baseline -level 3.0 -vf "scale=480:272:force_original_aspect_ratio=decrease,pad=480:272:(ow-iw)/2:(oh-ih)/2" -b:v 384k -acodec aac -ar 44100 -ab 128k`,
                    `-map 0:v -map 1:a`,
                    `-movflags +faststart`,
